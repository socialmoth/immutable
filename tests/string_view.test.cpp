...
TEST_CASE("default constructible"
          * doctest::description("tj::string_view is default constructible")
...
TEST_CASE(
    ...
    * doctest::description("tj::string_view can be constructed from a character pointer")
    * doctest::test_suite("string_view"))
{
...
TEST_CASE("character pointer construction"
          * doctest::description("tj::string_view can be constructed from tj::string")
          * doctest::test_suite("string_view"))
{
    const string s{"hello, world"};
    const string_view sv{s};
    CHECK(sv.data()
          == s.data()); // The tj::string constructor must point at the original string,
    CHECK(sv.size() == s.size()); // and the lengths must match.
}


TEST_CASE("character pointer construction"
          * doctest::description("tj::string_view can be constructed from std::string")
          * doctest::test_suite("string_view"))
{
...
TEST_CASE("copy construction"
          * doctest::description("tj::string_view can be copy-constructed")
          * doctest::test_suite("string_view"))
{
...
TEST_CASE("not constructible from nullptr"
          * doctest::description("tj::string_view cannot be constructed from nullptr")
          * doctest::test_suite("string_view"))
{
